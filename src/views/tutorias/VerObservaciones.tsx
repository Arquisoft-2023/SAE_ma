...
export function VerObservaciones({param1,param2}){
  ...
  const onGetUser = {
    userEmail: param1 ,
    userRol: rol[param2.charAt(0).toUpperCase() + param2.slice(1)]
  };
  const [user, setUser] = useState<myState["user"]>(onGetUser);
  if(onGetUser.userRol === rol.Bienestar) return (<Text>Acceso no valido...</Text>)
  const [getUser, {data, refetch}] = useLazyQuery((user.userRol === rol.Docente? obsQuery.obtenerAcompanyamientoTutor : obsQuery.obtenerAcompanyamientoEstudiante));
